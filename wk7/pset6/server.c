...
char* indexes(const char* path)
{
    // note this solution could fail if we had a file and dir with the same name
    // so, we won't ¯\_(ツ)_/¯
    
    // a stack variable with enough space for path + /index.html
    char *tmp_path = malloc(strlen(path) + 12);
    
    // store the result of path and concatenated "index.php" in tmp_path
    char *file = path[strlen(path) - 1] == '/' ? "index.php" : "/index.php";
    strcpy(tmp_path, path);
    tmp_path = strcat(tmp_path, file);
    // if the result is a file, return it
    if (access(tmp_path, F_OK) == 0)
    {
        char *index_path = malloc(strlen(tmp_path) + 1);
        strcpy(index_path, tmp_path);
        return index_path;
    }
    
    // store the result of path and concatenated "index.html" in tmp_path
    file = path[strlen(path) - 1] == '/' ? "index.html" : "/index.html";
    strcpy(tmp_path, path);
    tmp_path = strcat(tmp_path, file);
    // if the result is a file, return it
    if (access(tmp_path, F_OK) == 0)
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // loop through the file, getting length, reallocing and populating content
    int content_bytes = 32;
    *content = malloc(content_bytes);
    int len = 0;
    for (int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        ...
        {
            content_bytes *= 2;
            *content = realloc(*content, content_bytes);
            if (*content == NULL)
            {
                return false;
            }
...
const char* lookup(const char* path)
{
    // get a pointer to the final period
    char* ext_ptr = strrchr(path, '.');
    if (ext_ptr == NULL)
    {
        return NULL;
    }
    char *ext = malloc(strlen(ext_ptr));
    strcpy(ext, ext_ptr);
    
    // transform ext to lowercase for comparison
    for (int i = 0; ext[i] != '\0'; i++)
    {
        ext[i] = tolower(ext[i]);
    }
    
    // without using a switch statement because we can't :( conditionally
    // compare ext to set up our sweet MIME type response
    char *mimetype = NULL;
    if (strncmp(ext, ".css", 6) == 0)
    {
        mimetype = "text/css";
    }
    else if (strncmp(ext, ".html", 6) == 0)
    {
        mimetype = "text/html";
    }
    else if (strncmp(ext, ".gif", 6) == 0)
    {
        mimetype = "image/gif";
    }
    else if (strncmp(ext, ".ico", 6) == 0)
    {
        mimetype = "image/x-icon";
    }
    else if (strncmp(ext, ".jpg", 6) == 0)
    {
        mimetype = "image/jpeg";
    }
    else if (strncmp(ext, ".js", 6) == 0)
    {
        mimetype = "text/javascript";
    }
    else if (strncmp(ext, ".php", 6) == 0)
    {
        mimetype = "text/x-php";
    }
    else if (strncmp(ext, ".png", 6) == 0)
    {
        mimetype = "image/png";
    }
...
{
    // validate format for parsing
    int idx = 0;
    int spaces = 0;
    bool concurrent_spaces = false;
    while (line[idx] != '\0')
    {
        if (line[idx] == ' ')
        {
            ...
            if (idx > 0 && line[idx - 1] == ' ')
            {
                concurrent_spaces = true;
            }
        }
        idx++;
    }
    
    // bail with bad request if the structure is wrong
    if (spaces != 2 ||
        concurrent_spaces ||
        line[0] == ' ' ||
        line[idx - 2] != '\r' ||
    ...
    // extract the method, target and protocol
    char *method = malloc(LimitRequestLine + 1);
    char *target = malloc(LimitRequestLine + 1);
    char *protocol = malloc(LimitRequestLine + 1);
    sscanf(line, "%s %s %s\r\n", method, target, protocol);
    
    // bail if the method, target, or protocol are borked
    bool bail = false;
    ...
    // we should prob check for more than one ? but it's not in the spec
    // anyway, find the question mark
    char *qmark = strchr(target, '?');
    if (qmark == NULL)
    {
        // if its not there, the whole thing is a path
        for (int i = 0; target[i] != '\0'; i++)
        {
            abs_path[i] = target[i];
        }
        abs_path[strlen(target)] = '\0';
        query[0] = '\0';
    }
    else
    {
        // else, copy up to the qmark into abs_path
        int i = 0;
        while (target[i] != '?')
        {
            abs_path[i] = target[i];
        ...
        // then skip the question mark itself and copy the query
        qmark++;
        strcpy(query, qmark);
    }
